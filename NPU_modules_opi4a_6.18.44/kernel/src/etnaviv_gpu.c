...
static void etnaviv_hw_identify(struct etnaviv_gpu *gpu)
{
	...
	if (gpu->identity.model == 0x9000)
		gpu->idle_mask &= ~(VIVS_HI_IDLE_STATE_SH |
...
void etnaviv_gpu_recover_hang(struct etnaviv_gem_submit *submit)
{
	...
	dev_err(gpu->dev, "DMA_ADDR=0x%08x DMA_STATUS=0x%08x DEBUG=0x%08x IDLE=0x%08x INTR=0x%08x\n", gpu_read(gpu, VIVS_FE_DMA_ADDRESS), gpu_read(gpu, VIVS_FE_DMA_STATUS), gpu_read(gpu, VIVS_FE_DMA_DEBUG_STATE), gpu_read(gpu, VIVS_HI_IDLE_STATE), gpu_read(gpu, VIVS_HI_INTR_ACKNOWLEDGE));
	{
		u32 cva = etnaviv_cmdbuf_get_va(&submit->cmdbuf, &gpu->mmu_context->cmdbuf_mapping);
		u32 rva = etnaviv_cmdbuf_get_va(&gpu->buffer, &gpu->mmu_context->cmdbuf_mapping);
		dev_err(gpu->dev, "cmdbuf_va=0x%08x size=%u, ring_va=0x%08x user_size=%u\n", cva, submit->cmdbuf.size, rva, gpu->buffer.user_size);
		print_hex_dump(KERN_ERR, "cmdbuf: ", DUMP_PREFIX_OFFSET, 16, 4, submit->cmdbuf.vaddr, min(submit->cmdbuf.size, (size_t)512), 0);
		print_hex_dump(KERN_ERR, "ring: ", DUMP_PREFIX_OFFSET, 16, 4, gpu->buffer.vaddr, min((size_t)gpu->buffer.user_size, (size_t)512), 0);
	}
...
static irqreturn_t irq_handler(int irq, void *data)
{
	...
	pr_info("etnaviv_irq: intr 0x%08x\n", intr);
